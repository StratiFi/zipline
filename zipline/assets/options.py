...
class OptionChain(object):
    """ Allows users to look up option contracts.

    Parameters
    ----------
    asset_finder : AssetFinder
        An AssetFinder for option contract lookups, in particular the
    ...
    root_symbol : str
        The root symbol of a option chain.
    ...
    ----------
    root_symbol : str
        The root symbol of the option chain.
    as_of_date
        The current as-of date of this option chain.
    ...
    ------
    RootSymbolNotFound
        Raised when the OptionChain is initialized with a root symbol for which
        a option chain could not be found.
    """
    ...
    def __repr__(self):
        ...
        # exact OptionChain, since we don't want to display the asset
        # finder and get_datetime function to the user.
        if self._as_of_date:
            return "OptionChain(root_symbol='%s', as_of_date='%s')" % (
                self.root_symbol, self.as_of_date)
        else:
            return "OptionChain(root_symbol='%s')" % self.root_symbol
    ...
    def _get_datetime(self):
        ...
        -------
        pandas.Timestamp
            The normalized datetime of OptionChain's TradingAlgorithm.
        """
    ...
    def _maybe_update_current_chain(self):
        """ Updates the current chain if it's out of date, then returns
            ...
            -------
            list
                The up-to-date current chain, a list of Option objects.
        """
        if (self._last_updated is None)\
                or (self._last_updated != self.as_of_date):
            self._current_chain = self._asset_finder.lookup_option_chain(
                self.root_symbol,
                self.as_of_date
            )
    ...
    def as_of(self, dt):
        """ Get the option chain for this root symbol as of a specific date.
    ...
    def offset(self, time_delta):
        """ Get the option chain for this root symbol with a given
